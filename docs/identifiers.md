...
All categories of identifiers are different namespaces. E.g. a [script](scripts) and a [dialog](dialogs) may have the same name. This is why sigils are almost always necessary.
...
- `player`
	- The player entity (the entity with its [`is_player` flag](entities#player) set).
