...
Seriously, just use the [web build](what_youll_need#web-build) now! It's so much easier!
:::
...
The [encoder](encoder) is capable of generating a `game.dat` file from an arbitrary [`scenario_source_files/`](what_youll_need#scenario_source_files) folder, so having multiple project folders is not a problem.
