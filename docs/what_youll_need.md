...
The [web encoder](encoder#web-encoder) can be run with Node.js (see below) or in a web browser. They will both take the game files from your `scenario_source_files/` folder and export a `game.dat` for the Mage Game Engine to use.
...
This file contains data for portraits, which reference tileset JSON files from the `entities/` folder. Portrait names should match associated [entity_type](entity_types#character-entity) names, if any.
