...
4. Prepare dialog portraits.
	1. In Tiled, [[tilesets#Creating a Tileset JSON File|make a tileset JSON file]] for the talk portrait images.
		1. Put it in `scenario_source_files/entities/`.
	2. Prepare [[what_youll_need#`portraits.json`|`portraits.json`]].
	3. Assign portraits to their entities in [[what_youll_need#`entity_types.json`|`entity_types.json`]] if the portrait name does not match an [[entity_types#Character Entity|entity_type]] name.

## Maps

Prepare [[maps|map(s)]]. For each map:
...
5. [[vector_objects|Draw vector shapes]], e.g.
	...
	4. Camera targets
6. Add the map to [[what_youll_need#`maps.json`|`maps.json`]], and supply [[maps#Map Properties|properties]] like its `on_load` and `on_tick` scripts.
...
2. In Tiled, [[tilesets#Creating a Tileset JSON File|make a tileset JSON file]].
	1. Put it in `scenario_source_files/tilesets/`.
3. Add dialogSkin file(s) to [[what_youll_need#`scenario.json`|`scenario.json`]].
